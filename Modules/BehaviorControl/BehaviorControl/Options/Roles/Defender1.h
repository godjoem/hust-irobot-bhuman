 ...
 * @file Defender1.h
 *
 * This file defines an option that the left defender
 *
 * @author √◊Í≈Œƒ
 */
...
option(Defender1)
{
    initial_state(start)
    {
        transition
        {
            if (theRobotPose.translation.x() < -3500.f || theRobotPose.translation.x() > -2500.f || theRobotPose.translation.y() < -2000.f || theRobotPose.translation.x() > 2000.f)
                goto walkBack;
            if (state_time > 1000)
                goto waitBall;
    ...
    state(walkBack)
    {
        transition
        {
            if (theBallModel.estimate.position.norm() < 500.f)
                goto turnToBall;
            if ((theRobotPose.translation - Vector2f(-3000.f,0.f)).norm() < 100.f && std::abs(theLibCodeRelease.angleToGoal) < 5_deg)
                goto waitBall;
            if (theBallModel.estimate.position.norm() > 500.f && (theRobotPose.translation - Vector2f(-3000.f, theRobotPose.translation.y())).norm() < 500.f)
                goto walkLine;
        }
        action
        {
            HeadControlMode(HeadControl::lookForward);
            WalkToTarget(Pose2f(80.f, 80.f, 80.f), Pose2f(theLibCodeRelease.angleToGoal,(Vector2f(-3000.f, 0) - theRobotPose.translation)));
        }
    }
    state(walkLine)
    {
        transition
        {
            ...
            if (theBallModel.estimate.position.norm() > 500.f)
                goto walkLine;
            if ((theRobotPose.translation - Vector2f(-3000.f, theRobotPose.translation.y())).norm() > 500.f)
                goto walkBack;
        }
        action
        {
            HeadControlMode(HeadControl::lookForward);
            WalkToTarget(Pose2f(80.f, 80.f, 80.f), Pose2f(theBallModel.estimate.position.angle(),-3000.f, theBallModel.estimate.position.y()* (2500/theBallModel.estimate.position.x() + 4500)));
        }
    }
}
